...
Simulator::Simulator(int nMachines, int nRepairmen, unsigned int upperBound)
	...
	_mc(0), _rng(42)
{
	...
	_rng.setUpperBound(upperBound);
	for (int i = 0; i < _nMachines; i++)
		_events[i] = {i, _rng.generateOne()};
}
...
DataCollector	Simulator::run(unsigned int meanOT)
{
	this->_rng.setUpperBound(meanOT * 2);
	return (this->run());
}
...
DataCollector	Simulator::run(void)
{
	...
	while (schedule_event())
	{
		if (_eventId < _nMachines)
		{
			...
			_repairmen.addQueue({_eventId, _mc + _rng.generateOne() / 2.0});
		}
		else
		{
			...
			_events[machineId] = {machineId, _mc + _rng.generateOne()};
		}
		//_display();
		if (dc.calculateSizeDT() >= 1050 && c1 == c2)
			break ;
	}
	int i = 0;
	while (dc.calculateSizeDT() > 1000)
	{
