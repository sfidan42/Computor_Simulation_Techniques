...
RNG::RNG(int seed) : _max(NAN), _min(NAN), _avg(NAN), _upperBound(10)
{
	srand(seed);
}

double	RNG::generateOne(void)
{
	return ((double)rand() / RAND_MAX * _upperBound);
}
...
void	RNG::setUpperBound(unsigned int m)
{
	_upperBound = m;
}
